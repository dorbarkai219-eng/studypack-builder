...
describe("mock CoursePacks validate against the schema (spec §11)", () => {
  ...
  it("every derived deck slide references an existing block (plan↔deck integrity)", () => {
    for (const pack of [hebrewFinancePack, englishBiologyPack]) {
      const blockIds = new Set(pack.blocks.map((b) => b.id));
      for (const slide of buildDeck(pack)) {
        if (slide.blockId != null) expect(blockIds.has(slide.blockId)).toBe(true);
      }
    }
  });
});
